...
pub async fn find_all(pool: &PgPool) -> anyhow::Result<Vec<Project>> {
    let rows = sqlx::query_as::<_, Project>(
        "SELECT id, name, prefix, description, created_at FROM m_projects ORDER BY name"
...
pub async fn find_by_id(pool: &PgPool, id: i32) -> anyhow::Result<Option<Project>> {
    let row = sqlx::query_as::<_, Project>(
        "SELECT id, name, prefix, description, created_at FROM m_projects WHERE id=$1"
...
pub async fn create(pool: &PgPool, name: &str, prefix: &str, description: &str) -> anyhow::Result<i32> {
    let id = sqlx::query_scalar::<_, i32>(
        "INSERT INTO m_projects (name, prefix, description) VALUES ($1, $2, $3) RETURNING id"
...
pub async fn update(pool: &PgPool, id: i32, name: &str, prefix: &str, description: &str) -> anyhow::Result<()> {
    sqlx::query("UPDATE m_projects SET name=$2, prefix=$3, description=$4 WHERE id=$1")
...
pub async fn delete(pool: &PgPool, id: i32) -> anyhow::Result<()> {
    sqlx::query("DELETE FROM m_projects WHERE id=$1").bind(id).execute(pool).await?;
    Ok(())
}

pub async fn find_by_prefix(pool: &PgPool, prefix: &str) -> anyhow::Result<Option<Project>> {
    let row = sqlx::query_as::<_, Project>(
        "SELECT id, name, prefix, description, created_at, updated_at FROM m_projects WHERE prefix=$1"
